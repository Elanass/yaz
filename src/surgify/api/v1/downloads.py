...
async def download_ios_app():
    ...
    try:
        ...
        return {"message": "iOS app will be available on the App Store soon", "url": app_store_url}
    except Exception as e:
        logger.error(f"iOS download error: {e}")
        raise HTTPException(status_code=500, detail="App Store link temporarily unavailable")
...
async def download_android_app():
    ...
    try:
        ...
        return {"message": "Android app will be available on Google Play soon", "url": play_store_url}
    except Exception as e:
        logger.error(f"Android download error: {e}")
        raise HTTPException(status_code=500, detail="Play Store link temporarily unavailable")
