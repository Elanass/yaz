...
def get_recommendations(
    patient_data: dict,
    include_research: bool = Query(False, description="Include research-based recommendations (optional)"),
    ...
    try:
        ...
        if include_research and legacy_bridge and RESEARCH_AVAILABLE:
            try:
                ...
                enhanced_recommendations = legacy_bridge.enhance_recommendations_response(
                    {'recommendations': recommendations, 'case_id': case_data['case_id']}, 
                    ...
                    if isinstance(research_recs, dict):
                        for category, recs in research_recs.items():
                            if isinstance(recs, list):
                                recommendations.extend([f"[Research-Based] {rec}" for rec in recs])
...
def predict_outcome(
    patient_data: dict,
    include_research: bool = Query(False, description="Include research-based outcome prediction (optional)"),
    legacy_bridge: Optional[Union[LegacyBridge, None]] = Depends(get_legacy_bridge)
):
    """
        ...
        if include_research and legacy_bridge and RESEARCH_AVAILABLE:
            try:
                ...
                outcome['research_insights'] = {
                    'evidence_based_probability': research_prediction.get('primary_prediction', {}),
                    'confidence_level': research_prediction.get('evidence_metrics', {}).get('confidence_level'),
                    'similar_cases_analyzed': research_prediction.get('evidence_metrics', {}).get('sample_size', 0)
                }
