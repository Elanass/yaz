...
def search_procedures() -> List[SearchResult]:
    ...
    procedures = [
        {"id": "gastric_resection", "name": "Gastric Resection", "description": "Surgical removal of part or all of the stomach"},
        {"id": "laparoscopic_surgery", "name": "Laparoscopic Surgery", "description": "Minimally invasive surgical technique"},
        {"id": "bariatric_surgery", "name": "Bariatric Surgery", "description": "Weight loss surgery procedures"},
        {"id": "endoscopic_procedures", "name": "Endoscopic Procedures", "description": "Non-invasive diagnostic and therapeutic procedures"},
        {"id": "emergency_surgery", "name": "Emergency Surgery", "description": "Urgent surgical interventions"}
    ]
...
async def search(request: SearchRequest):
    ...
    procedure_results = []
    if any(query.lower() in proc.title.lower() or query.lower() in proc.description.lower() 
           for proc in search_procedures()):
        procedure_results = [proc for proc in search_procedures() 
                           if query.lower() in proc.title.lower() or query.lower() in proc.description.lower()]
...
async def get_search_suggestions(query: str):
    ...
    common_terms = [
        "gastric cancer", "emergency surgery", "laparoscopic", "bariatric",
        "endoscopic", "diagnosis", "high risk", "completed cases"
    ]
