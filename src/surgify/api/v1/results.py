...
async def get_results_summary(
    ...
    try:
        ...
        stage_counts = db.query(
            CohortData.stage, func.count(CohortData.id)
        ).group_by(CohortData.stage)
        ...
        domain_counts = db.query(
            CohortData.domain, func.count(CohortData.id)
        ).group_by(CohortData.domain).all()
        
        domain_distribution = {domain: count for domain, count in domain_counts}
        
        # Recent activity
        recent_cutoff = datetime.utcnow() - timedelta(hours=hours)
        recent_activity_query = base_query.filter(
            CohortData.created_at >= recent_cutoff
        ).order_by(CohortData.created_at.desc()).limit(10)
        ...
        while True:
            ...
            update = {
                ...
                "payload": {
                    ...
                    "processingRate": round(12 + (datetime.utcnow().timestamp() % 10), 2),
                    "avgRiskScore": round(3.4 + (datetime.utcnow().timestamp() % 5) * 0.1, 2)
                },
                "timestamp": datetime.utcnow().isoformat()
            }
...
async def export_as_csv(results: List, include_statistics: bool) -> StreamingResponse:
    ...
    # Write headers
    headers = ['patient_id', 'age', 'stage', 'gender', 'histology', 'location', 'domain', 'created_at']
    ...
    try:
        ...
        for i in range(limit):
            ...
            entry = {
                ...
                "severity": ["info", "warning", "error"][i % 3] if not severity else severity,
