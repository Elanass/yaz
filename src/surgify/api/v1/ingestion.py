...
async def ingest_data(
    ...
    try:
        ...
        logger.info(f"Starting data ingestion for upload_id: {upload_id}, domain: {domain}")
...
async def get_ingestion_status(upload_id: str, db: Session = Depends(get_db)):
    ...
    """
    try:
        ingestion_log = db.query(IngestionLog).filter(
            IngestionLog.upload_id == upload_id
        ).first()
...
async def get_recent_uploads(limit: int = 10, domain: Optional[str] = None, db: Session = Depends(get_db)):
    """
...
async def process_ingestion_data(data: List[Dict], domain: str, upload_id: str, db: Session):
    """
    ...
    try:
        ...
        for i, record in enumerate(data):
            try:
                ...
                if i % 10 == 0:  # Update every 10 records
                    update_ingestion_status(db, upload_id, "processing", progress, processed_count)
        ...
        update_ingestion_status(
            db, upload_id, final_status, 100, processed_count, 
            errors=errors, completed_at=datetime.utcnow()
        )
        
        logger.info(f"Completed processing upload_id: {upload_id}, processed: {processed_count}, errors: {len(errors)}")
...
def validate_and_clean_record(record: Dict, domain: str) -> Dict:
    ...
    cleaned = {
        ...
        'histology': record.get('histology', '').strip() if record.get('histology') else None,
        'location': record.get('location', '').strip() if record.get('location') else None,
    }
