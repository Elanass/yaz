...
class SummarizeRequest(BaseModel):
    ...
    medical_data: Optional[Dict[str, Any]] = Field(None, description="Additional medical data")
...
async def summarize_case(request: SummarizeRequest) -> SummarizeResponse:
    ...
    try:
        ...
        try:
            ...
            logger.info(f"✅ AI summary generated successfully for case {request.case_id} in {processing_time}ms")
            ...
            raise HTTPException(
                status_code=500,
                detail=f"AI service error: {str(openai_error)}"
            )
