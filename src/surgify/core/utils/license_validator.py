...
class LicenseValidator:
    ...
    def _extract_module_license(self, module_path: Path) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "requires_attention": "Proprietary" in license_info
                or "Commercial" in license_info,
                "type": "internal",
            }
    ...
    def generate_license_report(self) -> Dict:
        ...
        compatible_deps = sum(
            1 for d in dependencies.values() if d.get("compatible", False)
        )
        attention_deps = sum(
            1 for d in dependencies.values() if d.get("requires_attention", False)
        )

        total_modules = len(internal_modules)
        compatible_modules = sum(
            1 for m in internal_modules.values() if m.get("compatible", False)
        )
        attention_modules = sum(
            1 for m in internal_modules.values() if m.get("requires_attention", False)
        )
        ...
        report = {
            ...
            "summary": {
                ...
                "overall_compliance": (compatible_deps + compatible_modules)
                / (total_deps + total_modules)
                if (total_deps + total_modules) > 0
                else 0,
            },
            "dependencies": dependencies,
            "internal_modules": internal_modules,
            "recommendations": self._generate_recommendations(
                dependencies, internal_modules
            ),
        }
    ...
    def _generate_recommendations(
        self, dependencies: Dict, internal_modules: Dict
    ) -> List[str]:
        ...
        unlicensed_modules = [
            name
            for name, info in internal_modules.items()
    ...
    def save_report(self, output_path: Optional[Path] = None) -> Path:
        ...
        if output_path is None:
            output_path = (
                self.project_root / "docs" / "legal" / "license_scan_report.json"
            )
...
def main():
    ...
    if args.format == "json":
        ...
        print(
            f"Dependencies: {report['summary']['compatible_dependencies']}/{report['summary']['total_dependencies']} compatible"
        )
        print(
            f"Internal Modules: {report['summary']['compatible_internal_modules']}/{report['summary']['total_internal_modules']} compliant"
        )
