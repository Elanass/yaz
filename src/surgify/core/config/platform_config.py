...
class PlatformConfig:
    ...
    electricsql_secure: bool = os.getenv("ELECTRICSQL_SECURE", "False").lower() in (
        "true",
        "1",
        "yes",
    )
    ...
    offline_storage_mb: int = int(
        os.getenv("OFFLINE_STORAGE_MB", "100")
    )  # 100 MB for offline storage
    max_request_size_mb: int = int(
        os.getenv("MAX_REQUEST_SIZE_MB", "50")
    )  # 50 MB max request size
    ...
    file_encryption_enabled: bool = os.getenv(
        "FILE_ENCRYPTION_ENABLED", "True"
    ).lower() in ("true", "1", "yes")
    patient_data_encryption: bool = os.getenv(
        "PATIENT_DATA_ENCRYPTION", "True"
    ).lower() in ("true", "1", "yes")
    enable_audit_logging: bool = os.getenv("ENABLE_AUDIT_LOGGING", "True").lower() in (
        "true",
        "1",
        "yes",
    )
    data_retention_days: int = int(
        os.getenv("DATA_RETENTION_DAYS", "2555")
    )  # ~7 years for healthcare data
    enable_real_time_collaboration: bool = os.getenv(
        "ENABLE_REAL_TIME_COLLABORATION", "True"
    ).lower() in ("true", "1", "yes")

    # Cache and optimization settings
    enable_response_compression: bool = os.getenv(
        "ENABLE_RESPONSE_COMPRESSION", "True"
    ).lower() in ("true", "1", "yes")
    static_files_max_age: int = parse_env_var(
        "STATIC_FILES_MAX_AGE", 86400, int
    )  # 1 day in seconds
    ...
    # Feature flags
    enable_swagger: bool = os.getenv("ENABLE_SWAGGER", "True").lower() in (
        "true",
        "1",
        "yes",
    )
