...
class SurgeryParser(BaseParser):
    ...
    def get_schema_patterns(self) -> List[str]:
        ...
        return [
            "surgery", "surgical", "procedure", "operation", "patient",
            "diagnosis", "surgeon", "anesthesia", "case", "outcome",
            "preop", "postop", "complication", "asa_score", "los"
        ]
    ...
    def _parse_dataframe(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if surgery_columns.get("procedure"):
            result["summary"]["procedures"] = df[surgery_columns["procedure"]].value_counts().to_dict()
        
        if surgery_columns.get("outcome"):
            result["summary"]["outcomes"] = df[surgery_columns["outcome"]].value_counts().to_dict()
        ...
        if surgery_columns.get("asa_score"):
            result["summary"]["asa_distribution"] = df[surgery_columns["asa_score"]].value_counts().to_dict()
    ...
    def _parse_dict(self, data: dict) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "has_patient_info": any(key in data for key in ["patient_id", "patient_name", "mrn"]),
                "has_procedure_info": any(key in data for key in ["procedure", "surgery_type", "operation"]),
                "has_outcome_info": any(key in data for key in ["outcome", "result", "status"])
            }
        }
    
    def _parse_text(self, text: str) -> Dict[str, Any]:
        """Parse surgical text data"""
        surgical_keywords = ["surgery", "operation", "procedure", "patient", "diagnosis"]
