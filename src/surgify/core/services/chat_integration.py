...
class ChatIntegrationService(BaseService):
    ...
    async def initialize_chatwoot(self) -> bool:
        ...
        try:
            if not all([self.chatwoot_config['account_id'], 
    ...
    async def send_chatwoot_message(self, conversation_id: int, content: str, 
                                   message_type: str = 'outgoing') -> Dict[str, Any]:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, headers=headers, json=payload) as response:
                    if response.status == 200:
                        ...
                        logger.error(f"Failed to send Chatwoot message: {response.status} - {error_text}")
    ...
    async def send_discord_message(self, content: str, channel_id: str = None, 
                                  embed: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    async def create_discord_invite(self, max_age: int = 86400, max_uses: int = 10) -> Optional[str]:
        """Create Discord server invite link"""
        try:
            if not self.discord_config['bot_token'] or not self.discord_config['channel_id']:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, headers=headers, json=payload) as response:
                    if response.status == 200:
                        ...
                        error_text = await response.text()
                        logger.error(f"Failed to create Discord invite: {response.status} - {error_text}")
                        return None
    ...
    async def handle_case_notification(self, case_id: int, event_type: str, 
    ...
    def _format_case_message(self, case_id: int, event_type: str, data: Dict[str, Any]) -> str:
