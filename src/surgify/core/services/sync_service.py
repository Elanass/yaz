...
class SyncService(BaseService):
    ...
    def __init__(self, db: Session = None):
        super().__init__()
        self.db = db
        self._sync_jobs = {}  # In-memory store for sync jobs (would be database in production)
        self._messages = {}   # In-memory store for messages (would be database in production)
    ...
    async def create_sync_job(self, request: SyncRequest, user_id: Optional[str] = None) -> SyncResponse:
    ...
    async def create_message(self, request: MessageRequest, sender_id: Optional[str] = None) -> MessageResponse:
    ...
    async def list_messages(
        ...
        try:
            ...
            now = datetime.utcnow()
            messages = [msg for msg in messages if not msg.expires_at or msg.expires_at > now]
    ...
    async def get_message(self, message_id: str) -> Optional[MessageResponse]:
        ...
        try:
            ...
            if message and message.expires_at and message.expires_at <= datetime.utcnow():
    ...
    async def mark_message_read(self, message_id: str, user_id: Optional[str] = None) -> bool:
    ...
    async def get_sync_status(self, resource_type: str, resource_id: Optional[str] = None) -> Dict[str, Any]:
        """Get sync status for a resource"""
        try:
            jobs = [job for job in self._sync_jobs.values() 
                   if job.resource_type == resource_type and 
                   (not resource_id or job.resource_id == resource_id)]
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            active_jobs = len([job for job in self._sync_jobs.values() 
                             if job.status == SyncStatus.IN_PROGRESS])
            total_messages = len(self._messages)
            unread_messages = len([msg for msg in self._messages.values() 
    ...
    async def _resolve_conflict(self, local_data: Dict[str, Any], remote_data: Dict[str, Any], strategy: str = "newer_wins") -> Dict[str, Any]:
        ...
        try:
            if strategy == "newer_wins":
                ...
                for key, value in remote_data.items():
                    if key not in merged or remote_data.get("updated_at", datetime.min) > local_data.get("updated_at", datetime.min):
                        merged[key] = value
                return merged
    ...
    async def _fetch_external_data(self, resource_type: str, resource_id: Optional[str] = None) -> List[Dict[str, Any]]:
