...
class CaseService(BaseService):
    ...
    async def create_case(
        self, request: CaseCreateRequest, user_id: Optional[str] = None
    ) -> CaseResponse:
        ...
        try:
            ...
            if filters:
                ...
                if filters.procedure_type:
                    query = query.filter(
                        CaseModel.procedure_type == filters.procedure_type
                    )
            ...
            if sort_order.lower() == "desc":
                query = query.order_by(
                    desc(getattr(CaseModel, sort_by, CaseModel.created_at))
                )
            else:
                query = query.order_by(
                    asc(getattr(CaseModel, sort_by, CaseModel.created_at))
                )
    ...
    async def update_case(
        self, case_id: int, request: CaseUpdateRequest, user_id: Optional[str] = None
    ...
    async def _generate_recommendations(self, case: CaseModel) -> List[str]:
        ...
        if "cardiac" in case.procedure_type.lower():
            ...
            recommendations.extend(["DVT prophylaxis", "Physical therapy consultation"])
