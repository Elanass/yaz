...
class ComorbidityORM(Base):
    ...
    id = Column(PostgreSQLUUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    patient_id = Column(
        PostgreSQLUUID(as_uuid=True), ForeignKey("patients.id"), nullable=False
    )
...
class ADCIDecisionORM(Base):
    ...
    treatment_plan_id = Column(
        PostgreSQLUUID(as_uuid=True), ForeignKey("treatment_plans.id"), nullable=True
    )
...
class ClinicalOutcomeORM(Base):
    ...
    adci_decision_id = Column(
        PostgreSQLUUID(as_uuid=True), ForeignKey("adci_decisions.id"), nullable=True
    )
