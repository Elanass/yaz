...
class TumorCharacteristics(BaseModel):
    ...
    size_mm: Optional[float] = Field(
        None, ge=0, description="Tumor size in millimeters"
    )
    differentiation: Optional[str] = Field(
        None, description="Tumor differentiation grade"
    )
...
class LaboratoryResults(BaseModel):
    ...
    total_protein_g_dl: Optional[float] = Field(
...
class PerformanceStatus(BaseModel):
    ...
    karnofsky_score: Optional[int] = Field(
        None, ge=0, le=100, description="Karnofsky Performance Scale"
    )
    weight_loss_percent: Optional[float] = Field(
        None, ge=0, le=100, description="Weight loss percentage"
    )
    nutritional_risk: Optional[str] = Field(
...
class Patient(BaseEntity):
    ...
    # Current condition
    tumor_characteristics: Optional[TumorCharacteristics] = Field(
...
class TreatmentPlan(BaseEntity):
    ...
    surgical_procedure: Optional[SurgicalProcedure] = Field(
        None, description="Planned surgery"
    )

    # Scheduling
    planned_start_date: Optional[date] = Field(
        None, description="Planned treatment start"
    )
...
class ADCIDecision(BaseEntity):
    ...
    patient_id: UUID = Field(..., description="Associated patient ID")
    treatment_plan_id: Optional[UUID] = Field(
        None, description="Associated treatment plan"
    )

    # Decision inputs
    input_parameters: Dict[str, Union[str, float, int, bool]] = Field(
...
class ClinicalOutcome(BaseEntity):
    ...
    patient_id: UUID = Field(..., description="Associated patient ID")
    adci_decision_id: Optional[UUID] = Field(
        None, description="Associated ADCI decision"
    )

    # Outcome measures
    overall_survival_months: Optional[float] = Field(
        None, ge=0, description="Overall survival"
    )
    disease_free_survival_months: Optional[float] = Field(
        None, ge=0, description="Disease-free survival"
    )
    progression_free_survival_months: Optional[float] = Field(
        None, ge=0, description="Progression-free survival"
    )

    # Quality of life
    quality_of_life_score: Optional[float] = Field(
        None, ge=0, le=100, description="QoL score"
    )
    functional_status_change: Optional[str] = Field(
