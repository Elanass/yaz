...
class LegacyBridge:
    ...
    def enhance_case_response(self, case_data: Dict[str, Any], include_research: bool = False) -> Dict[str, Any]:
        """
        ...
        try:
            research_data = self.surgify_adapter.enhance_existing_case_data(case_data.get('case_number'))
            if research_data:
                enhanced_data['research_insights'] = research_data.get('research_insights', {})
                enhanced_data['research_metadata'] = research_data.get('research_metadata', {})
        except Exception as e:
            # Fail gracefully - return original data if research enhancement fails
            enhanced_data['research_error'] = f"Research enhancement unavailable: {str(e)}"
        
        return enhanced_data
    
    def enhance_cases_list_response(self, cases_data: List[Dict[str, Any]], include_research: bool = False) -> List[Dict[str, Any]]:
        """
    ...
    def enhance_dashboard_response(self, dashboard_data: Dict[str, Any], include_research: bool = False) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Fail gracefully
            enhanced_data['research_metrics_error'] = f"Research metrics unavailable: {str(e)}"
        
        return enhanced_data
    
    def enhance_recommendations_response(self, recommendations_data: Dict[str, Any], include_research: bool = False) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if case_id:
                research_recommendations = self._generate_research_recommendations(case_id)
                enhanced_data['research_recommendations'] = research_recommendations
        except Exception as e:
            # Fail gracefully
            enhanced_data['research_recommendations_error'] = f"Research recommendations unavailable: {str(e)}"
        
        return enhanced_data
    
    def bridge_existing_endpoint(self, endpoint_name: str, original_response: Any, request_params: Dict[str, Any]) -> Any:
        """
        ...
        if endpoint_name == 'get_case':
            ...
            return self.enhance_cases_list_response(original_response, include_research=True)
        elif endpoint_name == 'get_dashboard':
            return self.enhance_dashboard_response(original_response, include_research=True)
        elif endpoint_name == 'get_recommendations':
            return self.enhance_recommendations_response(original_response, include_research=True)
        else:
            # For unrecognized endpoints, just return original response
            return original_response
    
    def validate_backward_compatibility(self, endpoint_name: str, original_response: Any, enhanced_response: Any) -> bool:
        """
        Validates that enhanced response maintains backward compatibility
        Ensures all original fields are preserved
        """
        try:
            if isinstance(original_response, dict) and isinstance(enhanced_response, dict):
                ...
                for key in original_response.keys():
                    ...
                    if key not in ['research_insights', 'research_metadata', 'research_metrics', 'research_recommendations']:
                        if original_response[key] != enhanced_response[key]:
                            return False
                
                return True
            
            elif isinstance(original_response, list) and isinstance(enhanced_response, list):
                ...
                for orig_item, enh_item in zip(original_response, enhanced_response):
                    if not self.validate_backward_compatibility(endpoint_name, orig_item, enh_item):
                        return False
    ...
    def _calculate_research_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            research_cohort = self.surgify_adapter.get_research_cohort(research_eligible_criteria)
            
            return {
                'total_research_eligible_cases': len(research_cohort),
                'research_coverage_percentage': (len(research_cohort) / total_cases * 100) if total_cases > 0 else 0,
                'unique_procedure_types': len(set(case.get('procedure_type') for case in research_cohort if case.get('procedure_type'))),
                'average_complexity_score': self._calculate_average_complexity(research_cohort),
    ...
    def _generate_research_recommendations(self, case_id: str) -> Dict[str, Any]:
        ...
        try:
            enhanced_case_data = self.surgify_adapter.enhance_existing_case_data(case_id)
    ...
    def _calculate_average_complexity(self, research_cohort: List[Dict[str, Any]]) -> float:
    ...
    def _calculate_risk_distribution(self, research_cohort: List[Dict[str, Any]]) -> Dict[str, int]:
