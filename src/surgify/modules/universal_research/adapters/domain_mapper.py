...
class ConceptMapper:
    ...
    def map_surgical_to_universal(self, surgical_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            'domain_specific': {
                'surgical_specialty': surgical_data.get('research_metadata', {}).get('specialty'),
                'procedure_classification': self._classify_procedure(surgical_data.get('procedure_type')),
                'anatomical_system': self._identify_anatomical_system(surgical_data.get('procedure_type')),
                'surgical_approach': self._identify_approach(surgical_data.get('procedure_type'))
            },
            ...
            'process_metrics': {
                ...
                'complexity_level': surgical_data.get('research_metadata', {}).get('complexity_score'),
                'quality_indicators': self._extract_quality_indicators(surgical_data)
            }
        }
    
    def create_universal_cohort_definition(self, surgical_criteria: Dict[str, Any]) -> Dict[str, Any]:
        """
        Creates universal cohort definition that can be applied across domains
        """
        return {
            'inclusion_criteria': {
                'entity_characteristics': self._map_entity_characteristics(surgical_criteria),
                'outcome_requirements': self._map_outcome_requirements(surgical_criteria),
                'temporal_constraints': self._map_temporal_constraints(surgical_criteria)
            },
            'exclusion_criteria': {
                'risk_thresholds': self._map_risk_thresholds(surgical_criteria),
                'data_quality_requirements': self._map_quality_requirements(surgical_criteria)
            },
            'stratification_factors': {
                'primary_factors': self._identify_primary_stratification(surgical_criteria),
                'secondary_factors': self._identify_secondary_stratification(surgical_criteria)
            }
        }
    ...
    def _load_universal_concepts(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'outcome_categories': ['binary', 'continuous', 'categorical', 'time_to_event'],
            'risk_categories': ['low', 'moderate', 'high', 'critical'],
            'success_measures': ['effectiveness', 'efficiency', 'safety', 'satisfaction']
        }
    ...
    def _map_entity_type(self, procedure_type: Optional[str]) -> str:
        ...
        if any(term in procedure_lower for term in ['surgery', 'operation', 'procedure']):
            return 'surgical_intervention'
        elif any(term in procedure_lower for term in ['diagnostic', 'screening', 'test']):
    ...
    def _map_outcome_measure(self, surgical_data: Dict[str, Any]) -> str:
        ...
        outcome_category = surgical_data.get('research_metadata', {}).get('outcome_category')
    ...
    def _map_risk_factors(self, surgical_data: Dict[str, Any]) -> List[str]:
        ...
        risk_factors = surgical_data.get('research_insights', {}).get('risk_factors', [])
    ...
    def _map_success_metrics(self, surgical_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            'effectiveness': {
                'primary_outcome_achieved': surgical_data.get('research_metadata', {}).get('outcome_category') in ['excellent', 'good'],
                'outcome_probability': surgical_data.get('research_insights', {}).get('outcome_probability', 0.0)
            },
            'efficiency': {
                'resource_utilization': 'optimal' if surgical_data.get('status') == 'completed' else 'suboptimal',
                'process_efficiency': self._assess_process_efficiency(surgical_data)
            },
            'safety': {
                'risk_level': self._categorize_risk_level(surgical_data.get('risk_score')),
    ...
    def _assess_resources(self, surgical_data: Dict[str, Any]) -> str:
        ...
        complexity = surgical_data.get('research_metadata', {}).get('complexity_score', 1.0)
    ...
    def _extract_quality_indicators(self, surgical_data: Dict[str, Any]) -> Dict[str, Any]:
        """Extract quality indicators"""
        return {
            'completeness': 'complete' if surgical_data.get('status') == 'completed' else 'incomplete',
            'timeliness': 'on_schedule' if surgical_data.get('scheduled_date') else 'unscheduled',
    ...
    def _identify_secondary_stratification(self, criteria: Dict[str, Any]) -> List[str]:
        ...
        return criteria.get('secondary_stratification', ['age_group', 'comorbidity_burden'])
