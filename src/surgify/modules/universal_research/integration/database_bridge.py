...
class DatabaseBridge:
    ...
    def get_aggregated_research_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            metrics['status_distribution'] = {status: count for status, count in status_counts}
            
            # Cases by procedure type
            procedure_counts = self.db_session.query(
                Case.procedure_type,
                func.count(Case.id)
            ).group_by(Case.procedure_type).all()
            ...
            completed_cases = self.db_session.query(Case).filter(Case.status == 'completed').count()
            metrics['research_eligible_cases'] = completed_cases
            metrics['research_eligibility_rate'] = (completed_cases / metrics['total_cases'] * 100) if metrics['total_cases'] > 0 else 0
            
            # Risk score statistics
            risk_scores = self.db_session.query(Case.risk_score).filter(Case.risk_score.isnot(None)).all()
            if risk_scores:
    ...
    def validate_data_integrity(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if total_cases == 0:
                validation_results['data_quality_issues'].append('No cases found in database')
                return validation_results
            
            # Check for missing patient data
            cases_without_patients = self.db_session.query(Case).filter(Case.patient_id.is_(None)).count()
            if cases_without_patients > 0:
                validation_results['data_quality_issues'].append(f'{cases_without_patients} cases missing patient data')
            
            # Check for missing procedure types
            cases_without_procedure = self.db_session.query(Case).filter(Case.procedure_type.is_(None)).count()
            if cases_without_procedure > 0:
                validation_results['data_quality_issues'].append(f'{cases_without_procedure} cases missing procedure type')
            
            # Check for missing outcome data (completed cases without risk scores)
            completed_without_risk = self.db_session.query(Case).filter(
                Case.status == 'completed',
                Case.risk_score.is_(None)
            ).count()
            if completed_without_risk > 0:
                validation_results['data_quality_issues'].append(f'{completed_without_risk} completed cases missing risk assessment')
            
            # Check patient demographic completeness
            patients_without_age = self.db_session.query(Patient).filter(Patient.age.is_(None)).count()
            total_patients = self.db_session.query(Patient).count()
            if patients_without_age > total_patients * 0.1:  # More than 10% missing age
                validation_results['data_quality_issues'].append(f'High percentage of patients missing age data ({patients_without_age}/{total_patients})')
            
            # Calculate research readiness score
            validation_results['research_readiness_score'] = self._calculate_research_readiness_score(
                total_cases, cases_without_patients, cases_without_procedure, completed_without_risk, patients_without_age, total_patients
            )
            
            # Generate recommendations
            validation_results['recommendations'] = self._generate_data_quality_recommendations(validation_results)
            
        except Exception as e:
            validation_results['data_quality_issues'].append(f'Data validation error: {str(e)}')
    ...
    def _get_patient_demographics_stats(self) -> Dict[str, Any]:
        ...
        try:
            # Age statistics
            age_stats = self.db_session.query(
            ...
            ).filter(Patient.age.isnot(None)).first()
            
            # Gender distribution
            gender_dist = self.db_session.query(
                Patient.gender,
                func.count(Patient.id)
            ).filter(Patient.gender.isnot(None)).group_by(Patient.gender).all()
            
            # BMI statistics
            bmi_stats = self.db_session.query(
            ...
            return {
                'age_statistics': {
                    ...
                    'mean': float(age_stats.mean) if age_stats and age_stats.mean else None,
                ...
                'bmi_statistics': {
                    ...
                    'mean': float(bmi_stats.mean) if bmi_stats and bmi_stats.mean else None,
                    'min': float(bmi_stats.min) if bmi_stats and bmi_stats.min else None,
                    'max': float(bmi_stats.max) if bmi_stats and bmi_stats.max else None
                }
            }
    ...
    def _calculate_research_readiness_score(self, total_cases: int, missing_patients: int, 
                                          missing_procedures: int, missing_risk: int, 
                                          missing_age: int, total_patients: int) -> float:
        ...
        age_completeness = (total_patients - missing_age) / total_patients if total_patients > 0 else 0
    ...
    def _generate_data_quality_recommendations(self, validation_results: Dict[str, Any]) -> List[str]:
        ...
        if score < 50:
            recommendations.append('Critical data quality issues require immediate attention before research use')
        elif score < 75:
            recommendations.append('Data quality is acceptable but improvements would enhance research value')
        else:
            recommendations.append('Data quality is good for research purposes')
        
        if 'cases missing patient data' in str(validation_results['data_quality_issues']):
            recommendations.append('Link all cases to patient records for comprehensive analysis')
        
        if 'cases missing procedure type' in str(validation_results['data_quality_issues']):
            recommendations.append('Ensure all cases have documented procedure types')
        
        if 'completed cases missing risk assessment' in str(validation_results['data_quality_issues']):
            recommendations.append('Complete risk assessments for all finished cases')
        
        if 'patients missing age data' in str(validation_results['data_quality_issues']):
