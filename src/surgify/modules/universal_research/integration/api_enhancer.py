...
from surgify.modules.universal_research.engines.research_generator import ResearchGenerator
...
class ResearchAPIEnhancer:
    ...
    def _register_research_endpoints(self):
        ...
        async def analyze_cohort(
            ...
            try:
                ...
                total_cases = analysis_result.get('cohort_summary', {}).get('total_cases', 0)
                if total_cases < min_cases:
                    return {
                        'message': f'Insufficient cases found ({total_cases} < {min_cases} required)',
                        'suggestions': ['Broaden criteria', 'Extend date range', 'Lower minimum threshold']
                    }
        ...
        async def generate_research_study(
            ...
            try:
                ...
                study = generator.generate_research_study(study_request['inclusion_criteria'])
                ...
                raise HTTPException(status_code=500, detail=f"Research study generation failed: {str(e)}")
        ...
        async def generate_case_series(
            ...
            try:
                ...
                case_series = generator.generate_case_series(series_request.get('selection_criteria', {}))
                
                # Add metadata
                case_series['metadata'] = {
                    'generated_by': current_user.username,
                    'generation_date': datetime.now().isoformat(),
                    'series_title': series_request.get('series_title', 'Generated Case Series')
                }
                ...
                raise HTTPException(status_code=500, detail=f"Case series generation failed: {str(e)}")
        ...
        async def generate_clinical_guidelines(
            ...
            try:
                # Generate guidelines
                guidelines = generator.generate_clinical_guidelines(guidelines_request.get('criteria', {}))
                ...
                raise HTTPException(status_code=500, detail=f"Guidelines generation failed: {str(e)}")
        ...
        async def get_research_metrics(
            ...
            try:
                ...
                metrics = {
                    ...
                    'procedure_type_distribution': self._calculate_procedure_distribution(research_cohort),
                    'outcome_completeness': self._calculate_outcome_completeness(research_cohort),
                    'data_quality_metrics': self._calculate_data_quality_metrics(research_cohort),
                    'research_potential_score': self._calculate_research_potential(research_cohort)
                }
    ...
    def enhance_existing_endpoints(self, app):
        ...
        async def add_research_enhancements(request, call_next):
            ...
            include_research = request.query_params.get('include_research', 'false').lower() == 'true'
    ...
    def _calculate_procedure_distribution(self, cohort_data: List[Dict[str, Any]]) -> Dict[str, int]:
    ...
    def _calculate_outcome_completeness(self, cohort_data: List[Dict[str, Any]]) -> float:
        """Calculate percentage of cases with complete outcome data"""
        if not cohort_data:
            return 0.0
        
        complete_outcomes = len([
            case for case in cohort_data 
            if case.get('research_metadata', {}).get('outcome_category')
        ])
        
        return (complete_outcomes / len(cohort_data)) * 100
    
    def _calculate_data_quality_metrics(self, cohort_data: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    def _calculate_research_potential(self, cohort_data: List[Dict[str, Any]]) -> float:
        ...
        quality_score = sum(data_quality_metrics.values()) / len(data_quality_metrics) / 100 if data_quality_metrics else 0
        
        procedure_diversity = len(set(case.get('procedure_type') for case in cohort_data if case.get('procedure_type')))
        diversity_score = min(1.0, procedure_diversity / 10)  # Normalize to 10 different procedures
        
        # Weighted average
        research_potential = (sample_size_score * 0.4 + quality_score * 0.4 + diversity_score * 0.2) * 100
