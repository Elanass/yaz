...
from ..utils import (InstanceNotFoundError, ProviderError, get_backup_path,
                     parse_memory, retry_with_backoff, run, run_json,
                     validate_instance_name, wait_for_condition)
...
class MultipassProvider(BaseProvider):
    ...
    def list_instances(self) -> List[Instance]:
        ...
        try:
            ...
            for instance_data in result.get("list", []):
                ...
                instances.append(
                    Instance(
                        name=name, status=status, ip_address=ip_address, metadata={}
                    )
                )
    ...
    def snapshot_instance(self, name: str, snapshot_name: str) -> bool:
        ...
        return self.export_instance(
            name, get_backup_path() / f"{name}-{snapshot_name}.backup"
        )
