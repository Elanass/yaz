...
from ..utils import (InstanceNotFoundError, ProviderError,
                     extract_ip_from_addresses, get_backup_path, parse_memory,
                     retry_with_backoff, run, run_json, validate_instance_name,
                     wait_for_condition)
...
class IncusProvider(BaseProvider):
    ...
    def get_instance(self, name: str) -> Optional[Instance]:
        ...
        try:
            ...
            return Instance(
                name=name, status=status, ip_address=None, memory_usage=0, metadata={}
            )
