...
from .utils import (ProviderError, get_backup_path, load_yaml_file,
                    save_yaml_file)
...
def cmd_init(args):
    ...
    try:
        ...
        if provider.ensure_host():
            print(
                f"✓ Orchestration system initialized with {provider.__class__.__name__}"
            )
            return 0
        else:
...
def cmd_status(args):
    ...
    try:
        ...
        for instance in instances:
            memory_str = (
                f"{instance.memory_usage // (1024**2)}MB"
                if instance.memory_usage
                else "N/A"
            )
            print(
                f"{instance.name:<20} {instance.status.value:<12} {instance.ip_address or 'N/A':<15} {memory_str:<10}"
            )
...
def main():
    ...
    parser_switch.add_argument(
        "provider", choices=["incus", "multipass"], help="Provider to switch to"
    )
