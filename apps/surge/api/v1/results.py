...
async def get_results_summary(
    ...
    try:
        ...
        stage_distribution = dict(stage_counts.all())
        ...
        logger.exception(f"Failed to get results summary: {e!s}")
        raise HTTPException(
    ...
    try:
        ...
        while True:
            try:
                ...
                logger.exception(f"WebSocket error: {e!s}")
                await websocket.send_json(
                    {
                        "type": "error",
                        "message": str(e),
...
async def handle_subscription(websocket: WebSocket, payload: dict) -> None:
...
async def send_periodic_statistics(websocket: WebSocket) -> None:
    ...
    try:
        ...
        logger.exception(f"Error sending periodic statistics: {e!s}")


async def send_processing_updates(websocket: WebSocket) -> None:
    ...
    try:
        ...
        logger.exception(f"Error sending processing updates: {e!s}")
...
async def generate_report(
    ...
    try:
        ...
        logger.exception(f"Report generation failed: {e!s}")
...
async def get_processing_log(
    ...
    try:
        ...
        logger.exception(f"Failed to get processing log: {e!s}")
...
async def broadcast_update(update: ProcessingUpdate) -> None:
...
async def trigger_results_update(update_type: str, payload: dict[str, Any]) -> None:
