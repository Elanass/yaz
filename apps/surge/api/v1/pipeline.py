...
async def process_csv_complete_pipeline(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail=f"Invalid pipeline configuration: {e!s}"
            )
        ...
        logger.exception(f"Pipeline initiation failed: {e!s}")
        raise HTTPException(status_code=500, detail=f"Pipeline failed to start: {e!s}")
...
async def get_pipeline_status(job_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get pipeline status: {e!s}"
        )
...
async def get_pipeline_results(job_id: str):
    ...
    try:
        ...
        results = {
            ...
            # FIX: Point to the deliverables API download endpoint used across the platform
            "download_urls": {
                deliverable_id: f"/api/v1/deliverables/download/{deliverable_id}"
        ...
        logger.exception(f"Failed to get pipeline results: {e!s}")
        raise HTTPException(status_code=500, detail=f"Failed to get results: {e!s}")
...
async def download_pipeline_deliverable(deliverable_id: str):
    ...
    Delegates to the centralized in-memory store used by the Deliverables API.
    """
    try:
        ...
        # Stream based on format/content
        if d.format == DeliverableFormat.PDF and d.content:
            return StreamingResponse(
                io.BytesIO(d.content),
                media_type="application/pdf",
                headers={
                    "Content-Disposition": f"attachment; filename={d.metadata.title}.pdf"
                },
            )
        if d.format == DeliverableFormat.INTERACTIVE and d.html_content:
            return StreamingResponse(
                io.BytesIO(d.html_content.encode()),
                media_type="text/html",
                headers={
                    "Content-Disposition": f"inline; filename={d.metadata.title}.html"
                },
            )
        if d.format == DeliverableFormat.API and d.api_response is not None:
            return JSONResponse(d.api_response)
        if d.format == DeliverableFormat.PRESENTATION and d.html_content:
            return StreamingResponse(
                io.BytesIO(d.html_content.encode()),
                media_type="text/html",
                headers={
                    "Content-Disposition": f"inline; filename={d.metadata.title}_presentation.html"
                },
            )
...
async def list_pipeline_jobs(status: str | None = None, limit: int = 20):
...
async def cancel_or_delete_pipeline_job(job_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to cancel/delete job: {e!s}"
        )
...
async def process_pipeline_async(
    ...
    try:
        ...
        # NEW: Save into centralized store for cross-module access
        store.save_processing_result(job_id, processing_result)
        ...
        if config.generate_insights:
            ...
            try:
                ...
                job_status.warnings.append(f"Insight generation failed: {e!s}")
                insights = None
        else:
            insights = None
        ...
        for audience, format_type in deliverable_tasks:
            try:
                ...
                if insights:
                    ...
                    # NEW: Save deliverable to centralized store
                    store.save_deliverable(deliverable_id, deliverable)

                    logger.info(f"Deliverable generated: {deliverable_id}")

            except Exception as e:
                error_msg = f"Failed to generate {format_type.value} for {audience.value}: {e!s}"
        ...
        job_status.errors.append(error_msg)
        logger.exception(f"Pipeline failed for job {job_id}: {e!s}")
