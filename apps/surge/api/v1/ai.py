...
class SummarizeRequest(BaseModel):
    ...
    medical_data: dict[str, Any] | None = Field(
        None, description="Additional medical data"
    )
    patient_info: dict[str, Any] | None = Field(None, description="Patient information")
...
class SummarizeResponse(APIModel):
    ...
    model_used_: str = Field(
        ..., alias="model_used", description="AI model used for generation"
    )
...
class AnalysisResponse(APIModel):
    ...
    confidence_score: float = Field(..., description="Confidence score (0-1)")
    model_used_: str = Field(..., alias="model_used", description="AI model used")
    ...
    try:
        ...
        try:
            ...
            return SummarizeResponse(
                ...
                model_used_=response.model,
                case_id=request.case_id,
            )

        except openai.error.RateLimitError:
            logger.exception("OpenAI API rate limit exceeded")
            raise HTTPException(
                status_code=429,
                detail="AI service temporarily unavailable due to rate limiting. Please try again later.",
            )
        except openai.error.AuthenticationError:
            logger.exception("OpenAI API authentication failed")
            raise HTTPException(
                status_code=500,
                detail="AI service authentication failed. Please check API configuration.",
            )
        except Exception as openai_error:
            logger.exception(f"OpenAI API error: {openai_error!s}")
            raise HTTPException(
                status_code=500, detail=f"AI service error: {openai_error!s}"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to generate summary: {e!s}"
        )
