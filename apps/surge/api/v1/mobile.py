...
async def subscribe_to_app_updates(subscription: AppSubscription):
    ...
    existing = next(
        (
            sub
            for sub in app_subscribers
            if sub["email"] == subscription.email
            and sub["platform"] == subscription.platform
        ),
        None,
    )
    ...
    app_subscribers.append(
        {
            ...
            if hasattr(subscription, "timestamp")
            else None,
        }
    )

    logger.info(
        f"New app subscription: {subscription.email} for {subscription.platform}"
    )
...
async def submit_feedback(feedback: FeedbackRequest):
    ...
    logger.info(
        f"New feedback submission: {feedback.type} from {feedback.email or 'anonymous'}"
    )
...
async def get_feedback():
    ...
    return {
        ...
        "by_type": {
            "feedback": len(
                [f for f in feedback_submissions if f["type"] == "feedback"]
            ),
