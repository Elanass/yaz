"""Sync Service for Surge Platform
...
class SyncRequest(BaseModel):
    ...
    source_endpoint: str | None = None  # For backward compatibility
...
class MessageRequest(BaseModel):
    ...
    recipient_id: str | None = Field(None, alias="recipient")  # Support both
    title: str | None = "Sync Message"  # Make optional with default
...
class SyncService(BaseService):
    ...
    async def create_sync_job(
        self, request: SyncRequest, user_id: str | None = None
    ) -> SyncResponse:
        ...
        try:
            ...
            logger.exception(f"Error creating sync job: {e}")
            raise
    ...
    async def get_sync_job(self, job_id: str) -> SyncResponse | None:
        """Get a specific sync job."""
        try:
            return self._sync_jobs.get(job_id)
        except Exception as e:
            logger.exception(f"Error getting sync job {job_id}: {e}")
            raise
    ...
    async def cancel_sync_job(self, job_id: str) -> bool:
        ...
        try:
            ...
            logger.exception(f"Error cancelling sync job {job_id}: {e}")
            raise

    async def create_message(
        self, request: MessageRequest, sender_id: str | None = None
    ) -> MessageResponse:
    ...
    async def mark_message_read(
        ...
        try:
            ...
            logger.exception(f"Error marking message {message_id} as read: {e}")
            raise
    ...
    async def _start_sync_job(self, job_id: str) -> None:
        ...
        try:
            ...
            if job:
                ...
                job.updated_at = datetime.utcnow()
            logger.exception(f"Error in sync job {job_id}: {e}")
    ...
    async def cleanup_expired_messages(self) -> None:
        ...
        try:
            ...
            logger.exception(f"Error cleaning up expired messages: {e}")
    ...
    async def _process_in_batches(
        self, items: list[Any], batch_size: int = 1000
    ) -> None:
    ...
    async def _send_to_db(self, batch: list[Any]) -> None:
    ...
    async def process_sync_job(self, job_data: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            logger.exception(f"Error processing sync job {job_id}: {e}")
    ...
    async def _send_message(self, message_request: MessageRequest) -> None:
