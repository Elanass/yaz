...
class LogisticsParser(BaseParser):
    ...
    def parse(self, data: Any) -> dict[str, Any]:
        ...
        try:
            ...
            logger.exception(f"Error parsing logistics data: {e}")
            return {"error": str(e)}
    ...
    def _parse_text(self, text: str) -> dict[str, Any]:
        ...
        [word for word in location_keywords if word.lower() in text.lower()]

        # Assess text type
        if (
            any(word in text.lower() for word in ["purchase", "order", "po"])
            or any(
                word in text.lower() for word in ["delivery", "shipment", "tracking"]
            )
            or any(word in text.lower() for word in ["invoice", "payment", "billing"])
            or any(word in text.lower() for word in ["inventory", "stock", "count"])
            or any(word in text.lower() for word in ["quality", "inspection", "damage"])
        ):
            pass
    ...
    def _parse_image_list(self, images: list[str]) -> dict[str, Any]:
        ...
        categorized_images = {category: [] for category in logistics_image_types}
            ...
            "documentation_ratio": len(
                [
                    ...
                    if any(
                        ...
                        for keyword in ["pdf", "doc", "invoice", "receipt"]
                    )
                ]
            )
