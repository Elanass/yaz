...
class InsuranceParser(BaseParser):
    ...
    def parse(self, data: Any) -> dict[str, Any]:
        ...
        try:
            ...
            logger.exception(f"Error parsing insurance data: {e}")
            return {"error": str(e)}
    ...
    def _parse_text(self, text: str) -> dict[str, Any]:
        ...
        re.findall(policy_pattern, text, re.IGNORECASE)

        # Look for claim numbers
        claim_pattern = r"\b(?:claim|clm)[\s#:]*([A-Z0-9-]+)\b"
        re.findall(claim_pattern, text, re.IGNORECASE)

        # Look for monetary amounts
        currency_pattern = r"\$[\d,]+\.?\d*|€[\d,]+\.?\d*|£[\d,]+\.?\d*"
        re.findall(currency_pattern, text)

        # Look for percentages (deductibles, copays, etc.)
        percentage_pattern = r"\b\d+\.?\d*%"
        re.findall(percentage_pattern, text)
        ...
        medical_code_pattern = (
            r"\b(?:ICD|CPT)[-\s]*\d+\.?\d*[A-Z]?\b|\b[A-Z]\d{2}\.\d{1,2}\b"
        )
        re.findall(medical_code_pattern, text, re.IGNORECASE)

        # Assess text type
        if (
            any(word in text.lower() for word in ["claim", "settlement", "adjustment"])
            or any(word in text.lower() for word in ["policy", "coverage", "terms"])
            or any(
                word in text.lower() for word in ["medical", "diagnosis", "treatment"]
            )
            or any(
                word in text.lower()
                for word in ["investigation", "fraud", "suspicious"]
            )
            or any(word in text.lower() for word in ["denial", "approve", "decision"])
        ):
            pass
    ...
    def _parse_image_list(self, images: list[str]) -> dict[str, Any]:
        ...
        categorized_images = {category: [] for category in insurance_image_types}
        valid_extensions = {
            ".jpg",
            ".jpeg",
            ".png",
            ...
            "document_ratio": len(
                [
                    ...
                    if any(
                        ...
                        for keyword in ["pdf", "doc", "form", "policy"]
                    )
                ]
            )
            / max(len(valid_images), 1),
            "medical_image_ratio": len(
                [
                    img
                    for img in valid_images
                    if any(
                        keyword in img.lower()
                        for keyword in ["medical", "xray", "mri", "ct", "scan"]
                    )
                ]
            )
            / max(len(valid_images), 1),
