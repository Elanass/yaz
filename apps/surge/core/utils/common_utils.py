...
def format_date(date_input: str | datetime | None) -> str:
...
def get_auth_token(request=None, headers: dict[str, str] | None = None) -> str | None:
...
def log_performance(func):
    ...
    async def async_wrapper(*args, **kwargs):
        ...
        try:
            ...
            duration = (datetime.now() - start_time).total_seconds()
            logger.exception(f"{func.__name__} failed after {duration:.3f}s: {e}")
            raise
