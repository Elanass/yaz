...
def validate_patient_data(patient_data: Dict[str, Any], validation_type: str = "basic") -> Tuple[bool, List[str]]:
    """
    ...
    validation_fields = {
        ...
        "adci": common_required_fields + [
            "diagnosis", "lab_results", "surgical_history", 
            "comorbidities"
        ],
        "flot": common_required_fields + [
            "performance_status", "tumor_stage", "histology", 
            "comorbidities", "prior_treatments", "lab_results", 
            "imaging_findings"
        ],
        "comprehensive": common_required_fields + [
            "diagnosis", "lab_results", "surgical_history", 
            "comorbidities", "performance_status", "tumor_stage", 
            "histology", "prior_treatments", "imaging_findings",
            "vital_signs", "medications", "allergies"
        ]
    }
    
    # Get the required fields based on validation type
    required_fields = validation_fields.get(
        validation_type, 
        validation_fields["basic"]
    )
    ...
    if not is_valid:
        logger.warning(f"Patient data validation failed for type {validation_type}. Missing fields: {missing_fields}")
...
def validate_surgical_case(case_data: Dict[str, Any]) -> Tuple[bool, List[str]]:
    ...
    """
    required_fields = [
        "case_id", "patient_id", "surgery_type", 
        "diagnosis", "surgical_plan", "clinical_staging"
    ]
    ...
    if not is_valid:
        logger.warning(f"Surgical case validation failed. Missing fields: {missing_fields}")
