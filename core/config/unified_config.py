...
class UnifiedConfig(BaseSettings):
    ...
    secret_key: str = Field(
        default_factory=lambda: secrets.token_hex(32), env="SECRET_KEY"
    )
    jwt_secret: str = Field(
        default_factory=lambda: secrets.token_hex(32), env="JWT_SECRET"
    )
    encryption_key: str = Field(
        default_factory=lambda: secrets.token_urlsafe(32), env="ENCRYPTION_KEY"
    )
    token_expire_minutes: int = Field(
        default=1440, env="TOKEN_EXPIRE_MINUTES"
    )  # 24 hours
    ...
    database_url: str = Field(
        default="sqlite+aiosqlite:///data/database/gastric_adci.db", env="DATABASE_URL"
    )
    ...
    enable_response_compression: bool = Field(
        default=True, env="ENABLE_RESPONSE_COMPRESSION"
    )
    enable_real_time_collaboration: bool = Field(
        default=True, env="ENABLE_REAL_TIME_COLLABORATION"
    )

    # ===============================
    # Performance & Optimization
    # ===============================
    static_files_max_age: int = Field(
        default=86400, env="STATIC_FILES_MAX_AGE"
    )  # 1 day
