...
def pwa_features():
    ...
    return Template("pwa_features.html")

@app.get("/dicom")
def dicom_viewer():
    """DICOM MPR/3D Viewer"""
    return Template("dicom_viewer.html")

@app.get("/collaboration")
def collaboration():
    """WebRTC Collaboration & Telemedicine"""
    return Template("collaboration.html")

@app.get("/ar-visualization")
def ar_visualization():
    """AR Visualization for Surgical Planning"""
    return Template("ar_visualization.html")

@app.get("/post-op")
def post_op_monitoring():
    """Post-Op Patient Monitoring"""
    return Template("post_op.html")

@app.get("/analytics")
def ai_analytics():
    """AI Analytics and Predictive Modeling"""
    return Template("ai_analytics.html")

@app.get("/instrument-tracking")
def instrument_tracking():
    """Surgical Instrument Tracking"""
    return Template("instrument_tracking.html")
...
# PWA feature info endpoints for HTMX
@app.get("/pwa/dicom-info")
def dicom_info():
    return Div([
        H3("DICOM MPR/3D Visualization", cls="mb-3"),
        P("Advanced medical imaging viewer with multi-planar reconstruction (MPR) and 3D visualization capabilities. Powered by Cornerstone.js and WebGL."),
        Ul([
            Li("Load DICOM images with 3D/MPR support"),
            Li("Offline access to cached imaging data"),
            Li("HIPAA-compliant viewing environment"),
            Li("Measurement and annotation tools"),
            Li("Volume rendering for 3D visualization"),
            Li("Synchronize views across multiple clinicians")
        ])
    ])

@app.get("/pwa/collab-info")
def collaboration_info():
    return Div([
        H3("WebRTC Collaboration & Telemedicine", cls="mb-3"),
        P("Real-time telemedicine and surgical team collaboration using secure WebRTC and Gun.js for distributed state management."),
        Ul([
            Li("Encrypted peer-to-peer video conferencing"),
            Li("Collaborative medical image annotation"),
            Li("Persistent case discussion logs"),
            Li("Real-time chat with medical terminology support"),
            Li("HIPAA/GDPR compliant communication"),
            Li("Works on low-bandwidth connections")
        ])
    ])
...
def ar_info():
    return Div([
        H3("AR Visualization", cls="mb-3"),
        P("Augmented reality visualization for surgical planning and anatomical education using WebXR."),
        Ul([
            Li("3D models from patient CT/MRI data"),
            Li("Anatomical structure highlighting"),
            Li("Surgical approach visualization"),
            Li("Interactive tumor localization"),
            Li("Overlay virtual guides onto real-world environment"),
            Li("Collaborative AR sessions for team planning")
        ])
    ])

@app.get("/pwa/monitoring-info")
def monitoring_info():
    return Div([
        H3("Post-Op Monitoring", cls="mb-3"),
        P("Real-time post-operative patient monitoring with offline support and critical alert system."),
        Ul([
            Li("Continuous vital signs monitoring"),
            Li("ERAS protocol compliance tracking"),
            Li("Offline-first data collection"),
            Li("Background sync when connection restored"),
            Li("Automated early warning scoring"),
            Li("Complete audit trail of all observations")
        ])
    ])
...
def ai_info():
    return Div([
        H3("AI Analytics", cls="mb-3"),
        P("Predictive analytics with confidence intervals and explainable AI for clinical decision support."),
        Ul([
            Li("Predictive complication risk modeling"),
            Li("Length-of-stay estimation"),
            Li("Treatment outcome prediction"),
            Li("Confidence interval visualization"),
            Li("Explainable AI with evidence citation"),
            Li("Background processing with Web Workers")
        ])
    ])

@app.get("/pwa/instrument-info")
def instrument_info():
    return Div([
        H3("Instrument Tracking", cls="mb-3"),
        P("Surgical instrument tracking with HIPAA compliance and real-time status updates."),
        Ul([
            Li("Real-time instrument location tracking"),
            Li("Usage history and sterilization status"),
            Li("Procedural inventory management"),
            Li("Automatic HIPAA-compliant logging"),
            Li("Works offline with background sync"),
            Li("Integration with hospital inventory systems")
        ])
    ])
