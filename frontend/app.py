...
from .islands.cohorts import CohortUploadIsland, CohortResultsIsland
...
def cohorts_page(
    ...
    mock_cohorts = [
        {
            "id": "cohort-001",
            "study_name": "Gastric Cancer Surgical Outcomes Study",
            "description": "Retrospective analysis of surgical outcomes in T2-T3 gastric cancer patients",
            "status": "completed",
            "total_patients": 156,
            "uploaded_by": "Dr. Smith",
            "created_at": "2024-01-15",
            "last_processed": "2024-01-16",
            "engine_name": "adci"
        },
        {
            "id": "cohort-002", 
            "study_name": "FLOT Neoadjuvant Response Cohort",
            "description": "Analysis of response rates to FLOT protocol in advanced gastric cancer",
            "status": "processing", 
            "total_patients": 89,
            "uploaded_by": "Dr. Johnson",
            "created_at": "2024-01-18",
            "last_processed": "2024-01-18",
            "engine_name": "flot"
        },
        {
            "id": "cohort-003",
            "study_name": "High-Risk Patient Cohort",
            "description": "Decision support analysis for high-risk surgical candidates",
            "status": "failed",
            "total_patients": 42,
            "uploaded_by": "Dr. Wilson",
            "created_at": "2024-01-17",
            "last_processed": "2024-01-17",
            "engine_name": "gastrectomy"
        }
    ]
    
    # Filter cohorts if status provided
    if status:
        mock_cohorts = [c for c in mock_cohorts if c["status"] == status]
    
    content = Div(
        H1("Cohort Management", cls="text-3xl font-bold mb-6"),
        
        # Action buttons
        Div(
            A("Upload New Cohort", href="/cohorts/upload", cls="btn btn-primary"),
            A("View Active Studies", href="/cohorts?status=processing", cls="btn btn-outline"),
            cls="flex gap-4 mb-6"
        ),
        ...
        Div(
            *[Div(
                Div(
                    H3(cohort["study_name"], cls="text-xl font-semibold"),
                    P(cohort["description"], cls="text-gray-600 mb-2"),
                    Div(
                        Span(f"Status: {cohort['status'].title()}", 
                             cls=f"badge badge-{
                                 'success' if cohort['status'] == 'completed'
                                 else 'warning' if cohort['status'] == 'processing'
                                 else 'error'
                             }"),
                        Span(f"Patients: {cohort['total_patients']}", cls="badge badge-outline ml-2"),
                        Span(f"Engine: {cohort['engine_name'].upper()}", cls="badge badge-outline ml-2"),
                        cls="mb-3"
                    ),
                    Div(
                        A("View Results", href=f"/cohorts/{cohort['id']}/results", cls="btn btn-sm btn-primary"),
                        A("Export", href=f"/cohorts/{cohort['id']}/export", cls="btn btn-sm btn-outline ml-2"),
                        cls="flex gap-2"
                    ),
                    cls="p-4"
                ),
                cls="card bg-base-100 shadow-lg mb-4"
    ...
    return BaseLayout(
        title="Cohort Management - Gastric ADCI",
        content=content
    )

@app.get("/cohorts/upload")
def cohort_upload_page():
    """Cohort upload interface"""
    content = CohortUploadIsland.render()
    
    return BaseLayout(
        title="Upload Cohort - Gastric ADCI",
        content=content
    )

@app.get("/cohorts/upload-form/{format_type}")
def get_upload_form(format_type: str):
    """Dynamic upload form based on format type"""
    if format_type == "manual":
        return CohortUploadIsland.render_manual_form()
    else:
        return CohortUploadIsland.render_file_upload(format_type)

@app.get("/cohorts/{cohort_id}/results")
def cohort_results_page(cohort_id: str, session_id: str = None):
    """Cohort results visualization"""
    # Mock session data
    mock_session = {
        "cohort_id": cohort_id,
        "session_id": session_id or "session-001",
        "cohort_name": "Gastric Cancer Surgical Outcomes Study",
        "session_name": "Batch Processing - 2024-01-16 14:30",
        "status": "completed",
        "total_patients": 156,
        "engine_name": "adci",
        "progress": 100,
        "summary": {
            "total_patients": 156,
            "high_confidence_count": 134,
            "high_confidence_pct": 85.9,
            "avg_risk_score": 0.42,
            "processing_time_minutes": 12.5,
            "low_risk_count": 67,
            "low_risk_pct": 43.0,
            "medium_risk_count": 71,
            "medium_risk_pct": 45.5,
            "high_risk_count": 18,
            "high_risk_pct": 11.5
        }
    }
    
    content = CohortResultsIsland.render(cohort_id, mock_session)
    
    return BaseLayout(
        title=f"Results: {mock_session['cohort_name']} - Gastric ADCI",
        content=content
    )

@app.get("/cohorts/{cohort_id}/sessions")
def cohort_sessions_page(cohort_id: str):
    """List all processing sessions for a cohort"""
    # Mock sessions data
    mock_sessions = [
        {
            "id": "session-001",
            "session_name": "Initial Processing",
            "status": "completed",
            "total_patients": 156,
            "processed_at": "2024-01-16 14:30",
            "engine_name": "adci",
            "confidence_threshold": 0.75
        },
        {
            "id": "session-002", 
            "session_name": "High Confidence Re-analysis",
            "status": "completed",
            "total_patients": 156,
            "processed_at": "2024-01-17 09:15", 
            "engine_name": "adci",
    ...
    content = Div(
        H1("Processing Sessions", cls="text-3xl font-bold mb-6"),
        A("← Back to Cohort", href=f"/cohorts/{cohort_id}/results", cls="btn btn-outline mb-4"),
        
        Div(
            *[Div(
                H3(session["session_name"], cls="text-xl font-semibold"),
                Div(
                    Span(f"Status: {session['status'].title()}", cls="badge badge-success"),
                    Span(f"Patients: {session['total_patients']}", cls="badge badge-outline ml-2"),
                    Span(f"Engine: {session['engine_name'].upper()}", cls="badge badge-outline ml-2"),
                    cls="mb-3"
                ),
                P(f"Processed: {session['processed_at']}", cls="text-gray-600 mb-3"),
                A("View Results", href=f"/cohorts/{cohort_id}/results?session_id={session['id']}", cls="btn btn-primary btn-sm"),
                cls="card bg-base-100 shadow-lg p-4 mb-4"
    ...
    return BaseLayout(
        title="Processing Sessions - Gastric ADCI",
        content=content
    )
