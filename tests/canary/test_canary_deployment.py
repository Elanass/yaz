...
class CanaryHealthChecker:
    ...
    async def check_external_dependencies(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": "healthy" if response.status_code == 200 else "unhealthy",
                "dependencies": response.json() if response.status_code == 200 else None
            }
        except Exception as e:
            return {
                "status": "error",
...
class TestCanaryAPIFunctionality:
    ...
    async def test_canary_case_creation(self):
        ...
        response = await self.client.post(
            "/api/v1/cases/",
            json=CanaryTestConfig.TEST_CASE_DATA
        )
...
class TestCanaryPerformance:
    ...
    async def test_canary_response_times(self):
        ...
        for endpoint in endpoints_to_test:
            ...
            try:
                ...
                assert response_time < CanaryTestConfig.MAX_RESPONSE_TIME, \
        ...
        successful_requests = sum(1 for r in response_times.values() if r.get("success", False))
        success_rate = successful_requests / len(endpoints_to_test)
        
        assert success_rate >= CanaryTestConfig.MIN_SUCCESS_RATE, \
    ...
    async def test_canary_load_handling(self):
        ...
        successful_times = [r["response_time"] for r in flat_results if r.get("success", False) and "response_time" in r]
        avg_response_time = sum(successful_times) / len(successful_times) if successful_times else float('inf')
        
        # Performance assertions
        assert success_rate >= CanaryTestConfig.MIN_SUCCESS_RATE, \
            f"Load test success rate {success_rate} below threshold"
        
        assert avg_response_time < CanaryTestConfig.MAX_RESPONSE_TIME, \
...
class TestCanaryVsProduction:
            ...
            if not isinstance(canary_response, Exception):
                ...
                for field in key_fields:
                    if field in prod_data:
                        assert field in canary_data, f"Field '{field}' missing in canary response"
    ...
    async def test_canary_vs_production_api_structure(self):
        ...
        if not isinstance(canary_response, Exception) and not isinstance(prod_response, Exception):
            if canary_response.status_code == 200 and prod_response.status_code == 200:
                ...
                for field in essential_fields:
                    if field in prod_data:
                        assert field in canary_data, f"API field '{field}' missing in canary"
...
class CanaryTestRunner:
    ...
    async def run_full_canary_suite() -> Dict[str, Any]:
        ...
        results["overall_status"] = "passed" if success_rate >= CanaryTestConfig.MIN_SUCCESS_RATE else "failed"
        
        results["summary"] = {
            "total_tests": total_tests,
            "passed_tests": passed_tests,
            "success_rate": success_rate,
            "recommendation": "promote" if success_rate >= CanaryTestConfig.MIN_SUCCESS_RATE else "rollback"
        }
