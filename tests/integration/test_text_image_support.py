...
from apps.surge.core.models.insurance import InsuranceCaseModel
from apps.surge.core.models.logistics import LogisticsCaseModel
...
from apps.surge.core.models.surgery import SurgicalCaseModel
from apps.surge.core.parsers import detect_domain_from_data, get_parser_for_domain
...
async def test_domain_parsers():
    ...
    for domain in domains:
        ...
        print(
            f"    ✓ Text parsing: {text_result['data_type']}, likely_{domain}_text: {text_result.get(f'likely_{domain}_text', False)}"
        )
        ...
        print(
            f"    ✓ Image parsing: {image_result['data_type']}, valid images: {image_result.get('valid_image_count', 0)}"
        )
...
async def test_deliverable_generation():
    ...
    technical_analysis = TechnicalAnalysis(
        ...
        data_quality_notes=[
            ...
            "Some missing values in secondary endpoints",
        ],
    )
    ...
    from apps.surge.core.models.processing_models import AudienceType
    ...
    try:
        ...
        print(f"    ⚠️  Infographic generation skipped (missing dependencies): {e!s}")
