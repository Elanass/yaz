...
from surgify.api.v1.ingestion import (
    TextEntryData, MediaUploadData, IngestionResponse,
    TextEntryResponse, MediaUploadResponse
)
...
async def test_text_entry_api():
    ...
    for i, entry in enumerate(all_entries):
        ...
        try:
            ...
            print(f"    ✓ Entry validated: {len(entry.content)} characters, {len(entry.tags or [])} tags")
...
async def test_media_upload_api():
    ...
    for i, media in enumerate(all_media):
        ...
        try:
            ...
            print(f"    ✓ Media validated: {media.media_type}, {len(media.tags or [])} tags")
        except Exception as e:
