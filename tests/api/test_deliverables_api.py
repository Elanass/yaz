...
from surgify.core.services.deliverable_service import DeliverableType, DeliverableFormat, DeliverableStatus
...
class TestDeliverablesAPI:
    ...
    def test_create_deliverable_success(self, client, sample_deliverable_request):
        ...
        assert data["status"] in [DeliverableStatus.DRAFT, DeliverableStatus.PENDING_REVIEW]
    ...
    def test_list_deliverables_cache_scenarios(self, client, sample_deliverable_request):
        """Test list deliverables with cache miss then hit scenario"""
        # Create a deliverable first
        create_response = client.post("/api/v1/deliverables/", json=sample_deliverable_request)
    ...
    def test_list_deliverables_with_filters(self, client, sample_deliverable_request):
        ...
        # Test type filter
        response = client.get(f"/api/v1/deliverables/?type={sample_deliverable_request['type']}")
        ...
        # Test case_id filter
        response = client.get(f"/api/v1/deliverables/?case_id={sample_deliverable_request['case_id']}")
        assert response.status_code == 200
        data = response.json()
        for deliverable in data:
    ...
    def test_update_deliverable_success(self, client, sample_deliverable_request):
        ...
        response = client.put(f"/api/v1/deliverables/{deliverable_id}", json=update_data)
        assert response.status_code == 200
        data = response.json()
    ...
    def test_update_deliverable_status_transitions(self, client, sample_deliverable_request):
        ...
        response = client.put(f"/api/v1/deliverables/{deliverable_id}", json={"status": "approved"})
        assert response.status_code == 200
        data = response.json()
        ...
        response = client.put(f"/api/v1/deliverables/{deliverable_id}", json={"status": "published"})
        assert response.status_code == 200
        data = response.json()
    ...
    def test_update_deliverable_idempotent(self, client, sample_deliverable_request):
        ...
        response1 = client.put(f"/api/v1/deliverables/{deliverable_id}", json=update_data)
        response2 = client.put(f"/api/v1/deliverables/{deliverable_id}", json=update_data)
...
class TestDeliverablesPaginationAndFiltering:
    ...
    def test_concurrent_deliverable_operations(self, client, sample_deliverable_request):
