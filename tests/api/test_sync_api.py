...
class TestSyncJobsAPI:
    ...
    def test_create_sync_job_success(self, client, sample_sync_request):
        ...
        assert data["status"] in [SyncStatus.PENDING, SyncStatus.IN_PROGRESS, SyncStatus.COMPLETED]
    ...
    def test_list_sync_jobs_with_filters(self, client, sample_sync_request):
        ...
        response = client.get(f"/api/v1/sync/jobs?resource_type={sample_sync_request['resource_type']}")
        ...
        response = client.get(f"/api/v1/sync/jobs?target_system={sample_sync_request['target_system']}")
        assert response.status_code == 200
        data = response.json()
        for job in data:
...
class TestMessagesAPI:
    ...
    def test_list_messages_cache_scenarios(self, client, sample_message_request):
        ...
        create_response = client.post("/api/v1/sync/messages", json=sample_message_request)
        assert create_response.status_code == 201
    ...
    def test_list_messages_with_filters(self, client, sample_message_request):
        ...
        response = client.get(f"/api/v1/sync/messages?recipient_id={sample_message_request['recipient_id']}")
        assert response.status_code == 200
        data = response.json()
        for message in data:
        ...
        # Test type filter
        response = client.get(f"/api/v1/sync/messages?type={sample_message_request['type']}")
        assert response.status_code == 200
        data = response.json()
        for message in data:
...
class TestSyncStatusAPI:
    ...
    def test_get_sync_status_success(self, client, sample_sync_request):
        ...
        # Get sync status
        response = client.get(f"/api/v1/sync/status/{sample_sync_request['resource_type']}")
        assert response.status_code == 200
        data = response.json()
    ...
    def test_pagination_functionality(self, client, sample_sync_request, sample_message_request):
    ...
    def test_cache_invalidation_scenarios(self, client, sample_sync_request, sample_message_request):
