...
from core.models.base import PaginationParams, PaginationMeta, ApiResponse
...
class BaseRepository(ABC, Generic[T]):
        ...
        try:
            ...
            if filters:
                for field, value in filters.items():
                    if hasattr(self.model, field) and value is not None:
                        if isinstance(value, list):
                            count_query = count_query.where(getattr(self.model, field).in_(value))
                        else:
    ...
    async def update(self, entity_id: UUID, updates: Dict[str, Any], user_id: str = None) -> Optional[T]:
    ...
    async def search(
        ...
        try:
            ...
            await self._log_audit_event(
                action="search",
                user_id=user_id,
                new_values={"search_term": search_term, "fields": search_fields, "count": len(entities)}
            )
    ...
    async def exists(self, entity_id: UUID) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Error checking existence of {self.model.__name__} {entity_id}: {e}")
            raise
