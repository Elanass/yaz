...
class PatientRepository(BaseRepository[PatientORM]):
    ...
    async def get_by_mrn(
        self, medical_record_number: str, user_id: str = None
    ) -> Optional[PatientORM]:
        """Get patient by medical record number"""
        try:
            query = (
                select(PatientORM)
                .where(PatientORM.medical_record_number == medical_record_number)
                .options(
            ...
            self.logger.error(
                f"Error getting patient by MRN {medical_record_number}: {e}"
            )
            raise
    ...
    async def search_patients(
        ...
        try:
            ...
            if search_criteria.get("tumor_stage") or search_criteria.get(
                "tumor_histology"
            ):
                ...
                if search_criteria.get("tumor_stage"):
                    conditions.append(
                        TumorCharacteristicsORM.t_stage
                        == search_criteria["tumor_stage"]
                    )

                if search_criteria.get("tumor_histology"):
                    conditions.append(
                        TumorCharacteristicsORM.histology
                        == search_criteria["tumor_histology"]
                    )
            ...
            if conditions:
                if search_criteria.get("tumor_stage") or search_criteria.get(
                    ...
                    count_query = count_query.join(
                        TumorCharacteristicsORM, isouter=True
                    )
    ...
    async def get_patients_by_protocol(
        self, protocol: TreatmentProtocol, user_id: str = None
    ) -> List[PatientORM]:
        """Get all patients on a specific treatment protocol"""
        try:
            query = (
                select(PatientORM)
                .join(TreatmentPlanORM)
                .where(TreatmentPlanORM.protocol == protocol)
                .options(
                    selectinload(PatientORM.treatment_plans),
                    selectinload(PatientORM.tumor_characteristics),
                )
    ...
    async def get_patient_statistics(self, user_id: str = None) -> Dict[str, any]:
        ...
        try:
            ...
            # Gender distribution
            gender_query = select(
                PatientORM.gender, func.count(PatientORM.id)
            ).group_by(PatientORM.gender)
            gender_result = await self.session.execute(gender_query)
            gender_distribution = {
                gender.value: count for gender, count in gender_result.all()
            }
            ...
            age_query = select(
                ...
                .filter(and_(PatientORM.age >= 50, PatientORM.age < 70))
                .label("50_to_70"),
            ...
            await self._log_audit_event(
                action="get_statistics", user_id=user_id, new_values=statistics
            )
...
class TreatmentPlanRepository(BaseRepository[TreatmentPlanORM]):
    ...
    async def get_by_patient(
        self, patient_id: UUID, user_id: str = None
    ) -> List[TreatmentPlanORM]:
        """Get all treatment plans for a patient"""
        try:
            query = (
                select(TreatmentPlanORM)
                .where(TreatmentPlanORM.patient_id == patient_id)
            ...
            self.logger.error(
                f"Error getting treatment plans for patient {patient_id}: {e}"
            )
            raise
    ...
    async def get_active_plans(self, user_id: str = None) -> List[TreatmentPlanORM]:
        ...
        try:
            query = (
                select(TreatmentPlanORM)
                .where(TreatmentPlanORM.status == "active")
                .options(joinedload(TreatmentPlanORM.patient))
...
class ADCIDecisionRepository(BaseRepository[ADCIDecisionORM]):
    ...
    async def get_by_patient(
        self, patient_id: UUID, user_id: str = None
    ) -> List[ADCIDecisionORM]:
        """Get all ADCI decisions for a patient"""
        try:
            query = (
                select(ADCIDecisionORM)
                .where(ADCIDecisionORM.patient_id == patient_id)
            ...
            self.logger.error(
                f"Error getting ADCI decisions for patient {patient_id}: {e}"
            )
            raise
    ...
    async def get_by_decision_maker(
        ...
        try:
            query = (
                select(ADCIDecisionORM)
                .where(ADCIDecisionORM.decision_maker == decision_maker)
                .options(joinedload(ADCIDecisionORM.patient))
                .order_by(ADCIDecisionORM.decision_timestamp.desc())
            )
    ...
    async def get_decision_analytics(
        ...
        try:
            ...
            avg_score_result = await self.session.execute(
                select(func.avg(ADCIDecisionORM.adci_score)).select_from(
                    query.subquery()
                )
            )
            ...
            confidence_query = select(
                ADCIDecisionORM.confidence_level, func.count(ADCIDecisionORM.id)
            ).group_by(ADCIDecisionORM.confidence_level)

            if start_date:
                confidence_query = confidence_query.where(
                    ADCIDecisionORM.decision_timestamp >= start_date
                )
            if end_date:
                confidence_query = confidence_query.where(
                    ADCIDecisionORM.decision_timestamp <= end_date
                )

            confidence_result = await self.session.execute(confidence_query)
            confidence_distribution = {
                level.value: count for level, count in confidence_result.all()
            }

            # Implementation rate
            implementation_result = await self.session.execute(
                select(
                    func.count(ADCIDecisionORM.id).filter(
                        ADCIDecisionORM.implemented == True
                    )
                ).select_from(query.subquery())
            )
            implemented_count = implementation_result.scalar()
            implementation_rate = (
                (implemented_count / total_decisions * 100)
            ...
            await self._log_audit_event(
                action="get_analytics", user_id=user_id, new_values=analytics
            )
...
class ClinicalOutcomeRepository(BaseRepository[ClinicalOutcomeORM]):
    ...
    async def get_survival_statistics(
        self, protocol: TreatmentProtocol = None, user_id: str = None
    ) -> Dict[str, any]:
        ...
        try:
            ...
            survival_data = [
                o.overall_survival_months
                for o in outcomes
                if o.overall_survival_months is not None
            ]
            dfs_data = [
                o.disease_free_survival_months
                for o in outcomes
                if o.disease_free_survival_months is not None
            ]
            qol_data = [
                o.quality_of_life_score
                for o in outcomes
                if o.quality_of_life_score is not None
            ]
            ...
            statistics = {
                ...
                "overall_survival": {
                    ...
                    "mean": sum(survival_data) / len(survival_data)
                    if survival_data
                    else 0,
                    "median": sorted(survival_data)[len(survival_data) // 2]
                    if survival_data
                    else 0,
                },
            ...
            await self._log_audit_event(
                action="get_survival_statistics", user_id=user_id, new_values=statistics
            )
