...
def load_test_samples(cursor, test_samples_dir):
    ...
    # Sample users
    users = [
        ("dr_smith", "smith@hospital.com", "$2b$12$example_hash", "Dr. John Smith", "surgeon"),
        ("dr_johnson", "johnson@hospital.com", "$2b$12$example_hash", "Dr. Sarah Johnson", "surgeon"),
        ("dr_wilson", "wilson@hospital.com", "$2b$12$example_hash", "Dr. Michael Wilson", "surgeon")
    ]
    ...
    now = datetime.now()
    cases = [
        ("SURG-001", 1, 1, "gastric_resection", "Gastric adenocarcinoma", "planned", 
         now + timedelta(days=3), None, None, 0.4, 
         '["Consider neoadjuvant therapy", "Ensure nutritional support"]', ""),
        
        ("SURG-002", 2, 2, "laparoscopic", "Gastric ulcer perforation", "in_progress",
         now, now - timedelta(hours=1), None, 0.2,
         '["Monitor vital signs", "Prepare for conversion if needed"]', ""),
         
        ("SURG-003", 3, 1, "open_surgery", "Advanced gastric cancer", "completed",
         now - timedelta(days=1), now - timedelta(days=1, hours=2), 
         now - timedelta(days=1), 0.6,
         '["Adjuvant chemotherapy", "Regular follow-up"]', ""),
         
        ("SURG-004", 4, 2, "endoscopic", "Gastric polyp removal", "planned",
         now + timedelta(days=1), None, None, 0.1,
         '["Standard endoscopic procedure", "Biopsy recommended"]', ""),
         
        ("SURG-005", 5, 3, "gastric_bypass", "Obesity with gastric complications", "scheduled",
         now + timedelta(days=7), None, None, 0.3,
    ...
    # Sample protocols
    protocols = [
        ("Standard Gastric Resection", "1.0", "gastric_resection",
        ...
        ("Laparoscopic Surgery Protocol", "1.0", "laparoscopic",
        ...
        ("Emergency Surgery Protocol", "1.0", "emergency",
